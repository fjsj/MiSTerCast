...
inline size_t Rgb24FrameSize(uint32_t width, uint32_t activeHeight, bool interlaced)
{
    const uint32_t outputHeight = interlaced ? activeHeight / 2 : activeHeight;
...
{
    const size_t requiredSize = Rgb24FrameSize(outputWidth, activeHeight, interlaced);
    ...
    for (uint32_t y = 0; y < outputHeight; ++y)
    {
        ...
        const uint32_t fullY = interlaced ? y * 2 + !(field & 1) : y;
